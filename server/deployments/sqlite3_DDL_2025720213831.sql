...
)  ; --玩家账户;这张表考虑单独放一个身份校验服务器里，而各个角色的数据则用没有服务器标识号字段的子服务器存储（合并服务器可能性微存）
...
CREATE TABLE player_security(
    SRecordID INTEGER NOT NULL  , --日志ID
...
DROP TABLE IF EXISTS relation_planet_structure;
CREATE TABLE relation_planet_structure(
    planet_ID TEXT(32) NOT NULL  , --星球ID
...
DROP TABLE IF EXISTS relation_player_roles;
CREATE TABLE relation_player_roles(
    player_ID TEXT(32) NOT NULL  , --账户ID
    role_ID TEXT(32) NOT NULL  , --角色ID
...
CREATE TABLE fleet_carrier(
    ...
    carrier_SQUADRON_LIMIT INTEGER NOT NULL  , --舰载机上限
...
CREATE TABLE fleet_fleet(
    ...
    fleet_name TEXT(90) NOT NULL  , --舰队自定义名称
...
CREATE TABLE alliance_basic(
    alliance_ID TEXT(32) NOT NULL  , --ID
...
CREATE TABLE chat_mail(
    mail_SENDER TEXT(32) NOT NULL  , --邮件发送者
    mail_RECEIVER TEXT(32) NOT NULL  , --邮件接收者;考虑适配邮件群发
    mail_TITLE TEXT(900) NOT NULL  , --邮件标题
    mail_CONTENT TEXT(900) NOT NULL  , --邮件内容
    mail_SENDTIME NUMERIC NOT NULL  , --邮件发送时间
    mail_STATUS TEXT(32) NOT NULL   --邮件状态;考虑适配邮件群发
)  ; --私信表
...
DROP VIEW IF EXISTS view_system_planet;
CREATE VIEW view_system_planet AS
SELECT 
